...
## 最新预发布版：0.1.0-preview.20

`preview.20` 可从 [GitHub Releases](https://github.com/sincostancscseccot/KiriScope/releases/tag/v0.1.0-preview.20) 下载单文件 Windows x64 GUI。它面向普通用户提供“游戏目录 / XP3 / 完整游戏 ZIP → 资源类型 → 新导出目录 → 开始解包”的流程，不会修改输入游戏或压缩包。
...
- 运行时回退会优先选择直接导入 `VERSION.dll` 的 x86 引擎，而非按 EXE 体积误选受保护的启动器；失败报告会列出所选启动目标、代理声明的捕获数和实际落盘数。
...
- 想直接下载：前往 [0.1.0-preview.20 预发布](https://github.com/sincostancscseccot/KiriScope/releases/tag/v0.1.0-preview.20)。
