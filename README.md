...
## 最新预发布版：0.1.0-preview.21

`preview.21` 可从 [GitHub Releases](https://github.com/sincostancscseccot/KiriScope/releases/tag/v0.1.0-preview.21) 下载单文件 Windows x64 GUI。它面向普通用户提供“游戏目录 / XP3 / 完整游戏 ZIP → 资源类型 → 新导出目录 → 开始解包”的流程，不会修改输入游戏或压缩包。
...
- 运行时回退保留可启动启动器的优先级；`VERSION.dll` 导入与保护段仅作诊断，不会覆盖实际启动兼容性。它会跟踪启动器创建的子进程，并报告启动链、代理声明的捕获数和实际落盘数。
...
- 想直接下载：前往 [0.1.0-preview.21 预发布](https://github.com/sincostancscseccot/KiriScope/releases/tag/v0.1.0-preview.21)。
