...
## 最新预发布版：0.1.0-preview.27

`preview.27` 可从 [GitHub Releases](https://github.com/sincostancscseccot/KiriScope/releases/tag/v0.1.0-preview.27) 下载单文件 Windows x64 GUI。它面向普通用户提供“游戏目录 / XP3 / 完整游戏 ZIP → 资源类型 → 新导出目录 → 开始解包”的流程，不会修改输入游戏或压缩包。
...
- 运行时回退保留可启动启动器的优先级；`VERSION.dll` 导入与保护段仅作诊断，不会覆盖实际启动兼容性。它从引擎线程经游戏自己的存储 API（包括自定义 `archive://` 存储层）读取有效资源，不会从后台线程重入 KiriKiri，也不会把 XP3 原始密文当作解包结果。索引校验不一致的流仅在资源格式或脚本结构明确成立时导出，并在结果中单独标注。
- 已在获授权的受保护 KiriKiri 样本上验证真实路径恢复：输出名称保持可读且唯一，不会用哈希名或乱码替代已恢复的路径。
- 已在另一类带自定义存储层的获授权样本上完成端到端验证：可导出 PNG、OGG、TJS、MPEG 和字体等有效资源；非资源索引会明确跳过。不会再以“全部成功”掩盖未解码的字节流。
...
KiriScope 仅供学习、研究、格式兼容性分析，以及处理你拥有或已取得明确授权的副本。请遵守所在地法律、软件许可协议、著作权和数字内容保护规则。不得将本项目用于未经授权的解密、提取、复制、传播、规避技术保护措施，或任何其他违法用途。使用者须自行确认具备必要的权利与授权，并对其使用行为负责。
...
- 想直接下载：前往 [0.1.0-preview.27 预发布](https://github.com/sincostancscseccot/KiriScope/releases/tag/v0.1.0-preview.27)。
