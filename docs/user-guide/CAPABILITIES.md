...
`0.1.0-preview.23` 修复运行时捕获辅助 DLL 的两个已定位缺陷：不再把 KiriKiri 已解码的读取流再次送入标题的提取过滤器，也不再从后台计时线程调用 KiriKiri 存储 API。对于不调用提取过滤器回调的 Cx 变体，捕获会在引擎线程首次成功打开真实资源时启动一次。前者会使 Cx 资源被重新加密并触发 Adler-32 失败，后台线程会在标题界面阶段使受保护启动器崩溃；该版本仍只在每个请求流被实际写入、校验并确认后才报告导出成功。
