...
`0.1.0-preview.20` 改进了运行时回退的启动目标选择：优先直接导入 `VERSION.dll` 的 x86 引擎，降低带保护启动器段特征的候选优先级，并在失败结果中报告枚举、代理声明和文件落盘三个计数。它提高了“同目录含原始引擎与受保护启动器”场景的兼容性，但不保证所有运行时代理均可被未知作品接受。
