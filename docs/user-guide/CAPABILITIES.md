...
> **使用与安全声明：** 本项目仅供学习、研究和格式兼容性分析。请遵守所在地法律、软件许可协议、著作权和数字内容保护规则；不得用于未经授权的解密、提取、复制、传播、规避技术保护措施或其他违法用途。

## preview.27 已验证范围

`0.1.0-preview.19` 已验证能从获授权的受保护 KiriKiri 样本恢复 XP3 v3 `hnfn` 真实路径并写入唯一资源文件。该结果证明的是这一输入及其经验证的 Cx 路径，不是对所有 KiriKiri 作品的泛化承诺。未知的索引、文件名表或内容过滤器仍会被如实报告为无法处理。

`0.1.0-preview.21` 修正了运行时回退的启动目标策略：保留可启动启动器的优先级，`VERSION.dll` 导入和保护段特征只用于解释结果。它会观察启动器拉起的子进程，并在失败或成功结果中报告启动链、枚举、代理声明和文件落盘计数。它有助于定位多 EXE 游戏的实际资源读取链，但不保证所有运行时代理均可被未知作品接受。

`0.1.0-preview.25` 修复运行时捕获辅助 DLL 对“索引校验未确认”状态的清单遗漏：当引擎已完整返回并写入资源流、但索引中的 `FileHash` 不等于标准 Adler-32 时，辅助 DLL 会明确记录该状态，而不再把它静默丢失或当成写入失败。KiriScope 会对每个落盘流执行内容签名、结构验证和存在性检查；只有文件实际存在且后续验证/分类完成时才导出，真实写入失败仍会拒绝。校验不匹配的完整流只读取一次，避免在启动阶段重复处理。

`0.1.0-preview.27` 已在获授权的自定义存储层 KiriKiri 样本上执行全量运行时验证：它经游戏自身的 `archive://` 存储层导出经过验证的资源，而不是读取 XP3 原始记录。辅助 DLL 分别报告索引 Adler-32 确认流和索引校验不一致流；后者只有通过 PNG、OGG、MPEG、OpenType 或脚本结构检查后才会写入输出。每包附带的非资源版权提示仍会明确跳过。该结果只证明这个输入及其运行时路径，不代表未知 KiriKiri 变体必然兼容。
...
| 一键解包标准内容 | GUI 的“一键解包”标签，或 `unpack <游戏目录\|XP3\|游戏.zip> <新输出目录> --category <类别>` | 只读扫描游戏目录、独立 XP3 或完整游戏 ZIP，按“全部/图片/音频/脚本/其他”导出未标记加密的条目；多归档按来源保留目录结构。导出后会对受支持签名执行有上限的结构验证，并报告内容格式与路径类别是否一致。唯一的已验证 SHA-256 知识库命中会自动应用；内置静态 Cx 配置则必须在当前输入中由至少两条已标记条目的 Adler-32 校验交叉证明后才会启用。支持的 XP3 v3 `hnfn` 表会恢复真实路径，并对重复路径稳定加后缀。运行时回退会保留可启动启动器的优先级并记录其子进程链。ZIP 仅会按需暂存包内 XP3、EXE 或 DLL 用于指纹计算。 | ZIP 仅作为只读外层容器；限制条目数和解压体积，拒绝路径逃逸、重复路径、损坏或受保护的包。超出验证大小上限或缺少验证器的文件会明确标为未验证。弱匹配、未验证配置、缺少可验证路径表或多个并列命中绝不自动选择方案，也不会把不透明名称或密文报告成已成功解包。受保护启动器或非标准存储路径仍可能无法通过运行时回退捕获。 |
...
| 已验证方案过滤 | GUI 的“一键解包”或“验证所选方案”，以及 `xp3 extract --scheme`、`--xor-hex`、`filter score` | 对用户明确指定的方案应用内容过滤器并评分；一键流程还可使用经当前输入双重校验的内置静态 Cx 配置，或唯一的 SHA-256 知识库匹配。XP3 v3 `hnfn` 映射优先以 MD5 别名恢复文件名，Adler-32 仅作为后备，以减少碰撞。 | 不按目录名、游戏名或单条启发式证据自动猜测方案；具体游戏支持必须有可复核的样本、哈希和验证证据。 |
...
| 加密 XP3 | 支持用户明确指定的 XOR 或方案 JSON，以及少量经当前输入校验后启用的内置静态配置。 | 候选评分与提取；未知过滤器、索引或路径表会跳过并报告，而非伪称成功。 |
...
- 自动解开所有 KiriKiri 游戏、自动推导密钥、按文件名认定作品兼容性，或保证未知游戏的运行时后备可启动并恢复原始名称。
