...
`0.1.0-preview.24` 修复运行时捕获辅助 DLL 对非标准 XP3 索引校验的误判：当引擎已完整返回并写入资源流、但索引中的 `FileHash` 不等于标准 Adler-32 时，辅助 DLL 不再把文件当成写入失败。它会明确报告“索引校验未确认”，并由 KiriScope 对落盘流执行内容签名与结构验证；只有文件实际存在且后续验证/分类完成时才导出，真实写入失败仍会拒绝。
