...
`0.1.0-preview.19` 已验证能从获授权的《9-nine-天色天歌天籁音》样本恢复 XP3 v3 `hnfn` 真实路径并写入 9,982 个唯一资源文件。该结果证明的是这一输入及其经验证的 Cx 路径，不是对所有 KiriKiri 作品的泛化承诺。未知的索引、文件名表或内容过滤器仍会被如实报告为无法处理。
...
`0.1.0-preview.27` 已在获授权的《9-nine- 新章》目录执行全量运行时验证：它经游戏自身的自定义 `archive://` 存储层导出 7,241 个唯一有效资源，而不是读取 XP3 原始记录。辅助 DLL 分别报告索引 Adler-32 确认流和索引校验不一致流；后者只有通过 PNG、OGG、MPEG、OpenType 或脚本结构检查后才会写入输出。每包附带的非资源版权提示仍会明确跳过。该结果只证明这个输入及其运行时路径，不代表未知 KiriKiri 变体必然兼容。
