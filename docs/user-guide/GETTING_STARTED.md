...
若必须回退到游戏自身运行时，`0.1.0-preview.24` 会保留可启动启动器的优先级；`VERSION.dll` 导入与保护段特征仅供诊断，不会取代实际安装环境中的启动兼容性。运行时辅助 DLL 会在引擎线程首次成功打开真实资源时启动一次捕获，并直接写入引擎已经解码的流；若作品的 XP3 索引校验与标准 Adler-32 不一致，KiriScope 会把该流标记为“已写入、索引校验未确认”，然后通过内容签名和结构验证判断是否导出。结果会显示实际选择的 EXE、观察到的启动链、辅助 DLL SHA-256，以及枚举数、确认捕获数、校验未确认捕获数和落盘文件数。
