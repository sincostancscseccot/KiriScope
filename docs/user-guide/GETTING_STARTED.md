...
若必须回退到游戏自身运行时，`0.1.0-preview.23` 会保留可启动启动器的优先级；`VERSION.dll` 导入与保护段特征仅供诊断，不会取代实际安装环境中的启动兼容性。运行时辅助 DLL 会在引擎线程首次成功打开真实资源时启动一次捕获，并直接写入引擎已经解码的流；任务会继续观察启动器子进程。结果会显示实际选择的 EXE、观察到的启动链、辅助 DLL SHA-256，以及枚举数、代理声明的捕获数和落盘文件数，便于区分启动器链、代理路径与写入阶段的问题。
