...
> **合法使用声明：** 本项目仅供学习、研究和格式兼容性分析。使用前请确认你拥有必要的权利与授权，并遵守所在地法律、软件许可协议、著作权和数字内容保护规则。不得用于未经授权的解密、提取、复制、传播、规避技术保护措施或其他违法用途。
...
若必须回退到游戏自身运行时，`0.1.0-preview.27` 会保留可启动启动器的优先级；`VERSION.dll` 导入与保护段特征仅供诊断，不会取代实际安装环境中的启动兼容性。运行时辅助 DLL 会在引擎线程经游戏实际注册的存储 API（包括自定义 `archive://` 存储层）读取资源，而不会把 XP3 原始记录直接写出。若作品的 XP3 索引校验与标准 Adler-32 不一致，KiriScope 只会在内容格式或脚本结构明确成立时导出该流，并作单独标记。哈希索引名会依据 XP3 v3 `hnfn` 映射恢复为真实路径；映射到受保护归档版权提示的非资源项会明确跳过，而不会被报作捕获缺失。结果会显示实际选择的 EXE、观察到的启动链、辅助 DLL SHA-256，以及确认捕获数、结构确认数和落盘文件数。
