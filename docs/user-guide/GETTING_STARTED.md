...
若必须回退到游戏自身运行时，`0.1.0-preview.20` 会优先选择直接导入 `VERSION.dll` 的 x86 引擎，并降低带已知保护段特征的启动器优先级；它不再用“体积最大”作为主要依据。结果会显示实际选择的 EXE。若运行时枚举完成但资源没有写入，错误会分别列出枚举数、代理声明的捕获数及落盘文件数，便于判断是代理路径不兼容还是写入失败。
